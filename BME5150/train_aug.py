...
def train_model(
    ...
    for sample in original_dataset:
        augmented_sample = data_transform(sample)
        augmented_samples.append((sample, augmented_sample))

    random.shuffle(augmented_samples)
    augmented_dataset = [sample for _, sample in augmented_samples]

    n_val = int(len(dataset) * val_percent)
    n_train = len(dataset) - n_val

    train_set = augmented_dataset[:n_train]
    val_set = augmented_dataset[n_train:]
    ...
    # 创建新的数据集
    #train_set = torch.utils.data.ConcatDataset(augmented_samples)
    #train_set = augmented_samples
